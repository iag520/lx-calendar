	...
	<view class="date">
		...
		<swiper :style="{height:(retract ? 2 * 80 : (week_list.length + 1) * 80 ) + 'rpx'}" :current="current" circular @change="change_date">
			<swiper-item>
				...
				<view class="date_dl" v-show="!retract || index == to_prev_week_index" v-for="(item,index) in retract ?  week_list_prev_week : week_list_prev" :key="index">
			...
			<swiper-item>
				...
				<view class="date_dl" v-show="!retract || index == to_next_week_index" v-for="(item,index) in retract ?  week_list_next_week : week_list_next" :key="index">
					<view class="dd" @click="item_click(vo,index,key)" v-for="(vo,key) in item" :key="key">
						<view class="num" :class="[vo.today ? 'today' : '',vo.type == 'month' ? 'month' : (retract ? '' : 'disabled')]">{{vo.day}}</view>
						<view v-show="vo.dot" class="dot"></view>
					</view>
